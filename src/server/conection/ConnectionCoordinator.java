    ...
    public void run() {
        while (connectionState) {
            try {
                ...
                //PODRIA GENERAR ERROR AQUI CUANDO SE CIERRE EL SERVER    
                createStreams();
                //try to deserialize the string that user send
                Client client = (Client) Helper.deserialize(readMessage());
                client.setSocket(serviceSocket);
                //if success send the event to the server
                notifyOnClientConnect(this, new ClientConnectionEvent(client));
            } catch (IOException ex) {
                invalidate();
            } catch (ClassNotFoundException ex) {
                System.out.println("Can't deserialize the receive string from client when it tried to connect");
            } catch (Exception ex) {
